...
export function AssinaturaCarousel() {
  ...
  const getDisplayPrice = (monthlyPrice: number | null) => {
    ...
    if (billingCycle === "anual") {
      const annualTotal = (monthlyPrice * 12) * 0.90; // 10% de desconto para pagamento anual

      return `R$ ${annualTotal
        .toFixed(2)
        .replace(".", ",")}`;
    }
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-sm mx-auto pt-4 pb-6">
      {/* Toggle Mensal / Anual */}
      ...
      <div className="relative flex items-center justify-center w-full h-[360px]">
        {plans.map((plan, index) => {
          ...
          if (isCurrent) {
            positionClass =
              "opacity-100 z-20 scale-100 translate-x-0 shadow-2xl";
          } else if (isPrev) {
            positionClass =
          ...
          return (
            <div
              ...
              style={{
                background:
                  "linear-gradient(180deg, #1B263B 0%, #193160 100%)",
              }}
